...
public class RTSSelection : MonoBehaviour
{
  ...
  public KeyCode selectKey = KeyCode.LeftAlt;
  ...
  void Update()
  {
    ...
    if (Input.GetKey(selectKey))
    {
      if (Input.GetMouseButtonDown(0))
      {
        ...
        if (Physics.Raycast(mouseToWorldRay, out hitInfo))
        {
          ...
          RTSSelectable s = hitInfo.collider.GetComponentInParent<RTSSelectable>();
          if (s != null)
          {
    ...
    if (isSelecting)
    {
      ...
      foreach (RTSSelectable selectable in selectables)
      {
